...
Consumers of render output need both the generated resources and metadata about the module and instance — names, UUIDs, labels, and components. Module identity (what was built) and instance identity (how it was deployed) are semantically distinct: module UUID and instance UUID differ, and module name and instance name can differ when `--name` overrides the default.
...
- `ModuleMetadata` — Name, DefaultNamespace, FQN, Version, UUID, Labels, Annotations, Components. Fields like FQN and Version describe the source module and do not appear on the instance.
- `InstanceMetadata` — Name, Namespace, UUID, Labels, Annotations, Components. Fields describe the deployed instance and its runtime identity.
...
Delegate build-phase validation to `ModuleInstance` receiver methods (`ValidateValues()`, `Validate()`) rather than standalone functions, keeping validation co-located with the type it validates.
...
**Positive:** The module/instance metadata split prevents confusion between source identity and deployment identity, which matters when `--name` overrides or when the same module is released multiple times in an environment.
...
**Negative:** `RenderResult` has many fields — consumers must understand which fields are relevant to their use case and avoid drawing incorrect conclusions from fields that do not apply (for example, accessing instance UUID when only a build was requested).

**Negative:** Receiver-method delegation requires the `ModuleInstance` to be constructed earlier in the pipeline, which constrains the ordering of initialization steps.
