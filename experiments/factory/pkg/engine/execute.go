...
func executePair(
	...
	//   - list of resources
	//   - single resource (has "apiVersion" field)
...
//
// compVal should be the schema component (from rel.Schema) so that
...
func injectContext(
	...
	if annotationsVal := compVal.LookupPath(cue.ParsePath("metadata.annotations")); annotationsVal.Exists() {
		_ = annotationsVal.Decode(&compMeta.Annotations) // best-effort
	}
	unified = unified.FillPath(
