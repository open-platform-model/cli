...
provider: #ProviderWithMatching & {
	...
	transformers: {
		"deployment": core.#Transformer & {
			...
			#transform: {
				...
				let _ports = [
					if #component.spec.container.ports != _|_
					for _, port in #component.spec.container.ports {
						name:          port.name
				...
				let _env = [
					if #component.spec.container.env != _|_
					for _, env in #component.spec.container.env {
						name:  env.name
						value: env.value
					},
				]
	...
	// 2. Input the module
	#module: allBlueprintsModuleRelease
}

// 3. Output rendered platform resources
result: provider.#rendered

// Final rendered output as a stream of YAML documents
yaml: [
	for _, rendered in provider.#rendered {
		(rendered)
	},
]
