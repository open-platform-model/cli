...
func TestRegistryCheck_PrereleaseLineExempt(t *testing.T) {
	...
	broken := memberCatalogFilesAt("1.0.0-alpha.2")
	broken["resources/v1beta1/thing.cue"] = strings.Replace(
		broken["resources/v1beta1/thing.cue"], "size: int\n", "", 1)
	rawPush(t, registry, broken, "example.com/catalogs/demo@v1", "v1.0.0-alpha.2")

	report, err := checkFixture(t, registry, "example.com/catalogs/demo@v1.0.0-alpha.2", true)
	require.NoError(t, err)
	require.True(t, report.CompatRan)
	assert.True(t, report.Clean())
	assert.Equal(t, 4, report.Gates.CompatPrerelease)
	assert.Contains(t, report.Render(), "0 compared, 0 violating, 1 alpha-exempt, 4 prerelease-exempt, 0 new")

	// The stable build is compared against alpha.2, which already dropped
	// size; drop note? instead, a field alpha.2 still carries.
	...
	stable["resources/v1beta1/thing.cue"] = strings.Replace(
		stable["resources/v1beta1/thing.cue"], "note?: string\n", "", 1)
	rawPush(t, registry, stable, "example.com/catalogs/demo@v1", "v1.0.0")

	report, err = checkFixture(t, registry, "example.com/catalogs/demo@v1.0.0", true)
	require.NoError(t, err)
	require.False(t, report.Clean())
	assert.Equal(t, 0, report.Gates.CompatPrerelease)
	assert.Equal(t, 1, report.Gates.CompatRefused)
	var details string
	for _, f := range report.Findings {
		details += f.Headline + "\n" + f.Details() + "\n"
	...
	assert.Contains(t, details, "spec.note")
	assert.Contains(t, report.Render(), "4 compared, 1 violating, 1 alpha-exempt, 0 prerelease-exempt, 0 new")
}
