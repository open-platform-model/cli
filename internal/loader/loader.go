...
func Load(cueCtx *cue.Context, modulePath, registry string) (*core.Module, error) {
	...
	// load.Instances always returns at least one instance in practice;
	// this guard is a defensive fallback and is not reachable under normal conditions.
	if len(instances) == 0 {
	...
	// Step 7: Store the fully evaluated CUE value — required for BUILD phase FillPath injection
	mod.Raw = baseValue
