...
func TestBuildStatusOptions(t *testing.T) {
	rsf := &ReleaseSelectorFlags{ReleaseName: "demo", ReleaseID: "uuid-1"}
	inv := &inventory.InventorySecret{
		Index: []string{"change-1"},
		Changes: map[string]*inventory.ChangeEntry{
			"change-1": {
				Source:    inventory.ChangeSource{Version: "1.2.3"},
				Inventory: inventory.InventoryList{Entries: []inventory.InventoryEntry{{Kind: "Service", Namespace: "apps", Name: "web", Component: "frontend"}}},
			},
		},
	}
