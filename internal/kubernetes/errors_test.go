...
func TestNoResourcesFoundError(t *testing.T) {
	t.Run("error message with instance name", func(t *testing.T) {
		err := &noResourcesFoundError{
			InstanceName: "my-app",
			Namespace:    "production",
		}
		assert.Equal(t, `no resources found for instance my-app in namespace production`, err.Error())
	})

	t.Run("error message with instance-id", func(t *testing.T) {
		err := &noResourcesFoundError{
			InstanceID: "a1b2c3d4-e5f6-7890-abcd-ef1234567890",
			Namespace:  "production",
		}
		assert.Equal(t, `no resources found for instance-id a1b2c3d4-e5f6-7890-abcd-ef1234567890 in namespace production`, err.Error())
	})
	...
	t.Run("IsNoResourcesFound matches wrapped error", func(t *testing.T) {
		...
		wrapped := fmt.Errorf("discovering instance resources: %w", inner)
