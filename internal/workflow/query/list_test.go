...
func TestBuildReleaseSummary(t *testing.T) {
	...
	inv := &inventory.InventorySecret{ReleaseMetadata: inventory.ReleaseMetadata{ReleaseName: "demo", ReleaseNamespace: "apps", ReleaseID: "uuid-1", LastTransitionTime: now}, ModuleMetadata: inventory.ModuleMetadata{Name: "module-a"}, Index: []string{"change-1"}, Changes: map[string]*inventory.ChangeEntry{"change-1": {Source: inventory.ChangeSource{Version: "0.1.0"}, Timestamp: now}}}
