...
func Release(ctx context.Context, opts ReleaseOpts) (*Result, error) { //nolint:gocyclo
	...
	if !hasReleaseFile(modulePath) {
		...
		case opts.DebugValues:
			...
			if !debugVal.Exists() {
				return nil, &oerrors.ExitError{Code: oerrors.ExitGeneralError, Err: fmt.Errorf("no release.cue found - add debugValues to module or use -f <values-file>")}
			}
		...
		default:
			return nil, &oerrors.ExitError{Code: oerrors.ExitGeneralError, Err: fmt.Errorf("no release.cue found - use -f <values-file> to provide values")}
		}
	...
	for _, r := range engineResult.Resources {
		...
		if convErr != nil {
			return nil, &oerrors.ExitError{Code: oerrors.ExitGeneralError, Err: fmt.Errorf("converting resource %s/%s to unstructured: %w", r.Kind(), r.Name(), convErr)}
		}
...
func ReleaseFile(ctx context.Context, opts ReleaseFileOpts) (*Result, error) { //nolint:gocyclo
	...
	if fileRelease.Kind == internalreleasefile.KindBundleRelease {
		return nil, &oerrors.ExitError{Code: oerrors.ExitGeneralError, Err: fmt.Errorf("bundle releases are not yet supported - use a #ModuleRelease file")}
	}
	...
	if opts.ModulePath != "" {
		...
		if modErr != nil {
			return nil, &oerrors.ExitError{Code: oerrors.ExitGeneralError, Err: fmt.Errorf("loading module from --module: %w", modErr)}
		}
		...
		if err := rel.RawCUE.Err(); err != nil {
			return nil, &oerrors.ExitError{Code: oerrors.ExitGeneralError, Err: fmt.Errorf("filling #module from --module: %w", err)}
		}
	}
	...
	if !moduleVal.Exists() || moduleVal.Validate(cue.Concrete(true)) != nil {
		if opts.ModulePath == "" {
			return nil, &oerrors.ExitError{Code: oerrors.ExitGeneralError, Err: fmt.Errorf("#module is not filled in the release file - either import a module or use --module <path>")}
		}
	}
