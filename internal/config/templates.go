...
//
// This template is shipped content (every `opm config init` writes it), so
// a pin bump here commits as `fix(deps)` and releases the CLI; the mirrored
// fixture copies commit as `test(fixtures)`.
