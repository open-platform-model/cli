...
import (
	resources_extension "opmodel.dev/resources/extension@v1"
...
#components: {
	...
	controller: {
		...
		spec: {
			...
			container: {
				...
				// Cert volume mount — controller writes the TLS cert here at startup.
				volumeMounts: {
					cert: {
						name:      "cert"
			...
			// Webhook TLS cert — pre-created as empty; controller writes cert on startup.
			volumes: {
				cert: {
					name: "cert"
					secret: {
						from: #config.controller.webhookCertSecret
					}
				}
			}
	...
	speaker: {
		...
		spec: {
			...
			// Memberlist gossip encryption key — pre-created by the operator.
			volumes: {
				memberlist: {
					name: "memberlist"
					secret: {
						from: #config.speaker.memberlistSecret
					}
				}
			}
