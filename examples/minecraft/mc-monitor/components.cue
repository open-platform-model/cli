...
// Single stateless component that monitors Minecraft server status
// and exports metrics via Prometheus or OpenTelemetry.
package main
...
#components: {
	...
	monitor: {
		...
		// Only embed Expose trait if Prometheus mode (has an inbound port to expose).
		// OTel mode is push-based — no inbound service needed.
