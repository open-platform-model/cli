...
//
// DEBT #13: uses extractReleaseMetadata (shared with module_release.go) instead
// of the duplicated decodeModuleReleaseEntry helper that existed in the factory.
func extractBundleReleases(cueCtx *cue.Context, schemaPkg cue.Value) (map[string]*modulerelease.ModuleRelease, error) {
	...
	for _, key := range keys {
		...
		// DEBT #13: reuse extractReleaseMetadata (shared with module_release.go)
		// instead of a duplicate decodeModuleReleaseEntry helper.
		relMeta, err := extractReleaseMetadata(schemaEntry, key)
		if err != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("release %q: extracting module info: %w", key, err)
		}

		releases[key] = modulerelease.NewModuleRelease(relMeta, module.Module{
			Metadata: modMeta,
			Raw:      modRaw,
		}, schemaEntry, dataComponents)
	}
