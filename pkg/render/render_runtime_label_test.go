...
// passed to ProcessModuleRelease, and that the value reaches rendered resources
...
// TestRender_RuntimeName_EmptyFailsAtBoundary verifies that ProcessModuleRelease
...
func TestRender_RuntimeName_EmptyFailsAtBoundary(t *testing.T) {
	...
	_, err := ProcessModuleRelease(context.Background(), rel, p, "")
...
// future code path that bypasses ProcessModuleRelease entirely.
//
