...
Defines the `opm mod events` command that aggregates Kubernetes events from all resources belonging to an OPM release into a single chronological view, including events from Kubernetes-owned children (Pods, ReplicaSets).
