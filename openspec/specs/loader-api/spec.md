...
The `pkg/releaseprocess` package SHALL export a `SynthesizeModuleRelease` function that constructs a `*modulerelease.ModuleRelease` from a loaded module CUE value and one or more concrete values CUE values, without requiring a `release.cue` file.

The function signature SHALL be:
```
SynthesizeModuleRelease(cueCtx *cue.Context, modVal cue.Value, values []cue.Value, releaseName string, namespace string) (*modulerelease.ModuleRelease, error)
```

The function SHALL:
1. Run the Module Gate: validate the supplied values against `modVal.LookupPath("#config")` using `ValidateConfig`
