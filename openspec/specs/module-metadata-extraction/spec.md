...
Specifies how `module.Load` extracts all scalar metadata fields (`Name`, `DefaultNamespace`, `FQN`, `Version`, `UUID`, `Labels`) from the fully evaluated CUE value using `LookupPath`, without AST inspection. Supersedes the former AST-based name extraction approach.
