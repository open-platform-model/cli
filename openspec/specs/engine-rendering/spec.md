...
Defines the contract for `pkg/engine` — the rendering engine that takes pre-loaded releases and produces `[]*core.Resource`. Replaces the five-phase pipeline with two concrete renderer structs: `ModuleRenderer` and `BundleRenderer`.
