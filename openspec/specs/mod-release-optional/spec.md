# Capability: mod-release-optional

## Purpose

Defines the behavior of `opm mod build` and `opm mod apply` when no `release.cue` file is present in the module directory. These commands SHALL synthesize a `ModuleRelease` in memory from the module's `debugValues` or an explicit `-f` values file, enabling a developer workflow that does not require a `release.cue` file.
