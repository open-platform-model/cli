...
Defines public ownership policy checks in `pkg/ownership/` so both the CLI and external consumers (e.g., controllers) can enforce the no-takeover policy without importing internal packages.
