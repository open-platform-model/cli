...
> Slice `cli-login` of enhancement `0011`. Decision D11. Independent of everything else in the entry; `catalogs-publish-cutover` depends on it.
...
- **Commands**: `internal/cmd/login.go` (top-level, seventh root command); does not set `SkipConfigLoadAnnotation` (needs the resolved registry; config load does no Kubernetes work).
