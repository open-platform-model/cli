...
Type detection reads the `kind` field from the evaluated CUE value. Both `#ModuleRelease` (`kind: "ModuleRelease"`) and `#BundleRelease` (`kind: "BundleRelease"`) define `kind` as a concrete string literal in the OPM catalog.
