...
The `PrintRenderErrors` function SHALL accept a slice of errors and print each one with appropriate diagnostic information. For an unmatched-components refusal it SHALL print the component names and, for each, the candidate transformers the render build evaluated with the required labels each was found to lack. For an over-subscription refusal it SHALL print every contract key and the catalogs competing for it from the single typed cause that carries them all. For transform errors, it SHALL print the component name, transformer FQN, and cause. Unresolved demands SHALL be rendered from the demand rows themselves, without reconstructing an aggregate error to reach the formatter.

#### Scenario: Unmatched components error with diagnostics

- **WHEN** `PrintRenderErrors` is called with an unmatched-components refusal
- **THEN** the output SHALL include the unmatched component names
- **AND** for each, the candidate transformers evaluated for it with the labels the predicate found missing

#### Scenario: Over-subscribed contracts print once

- **WHEN** `PrintRenderErrors` is called with a refusal carrying two over-subscribed contract keys
