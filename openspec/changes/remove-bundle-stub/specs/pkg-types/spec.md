...
**Reason**: The bundle path is removed (enhancement 0002 D15, supersedes D7). The `pkg/bundle` package — `Bundle`, `BundleMetadata`, `Release`, `ReleaseMetadata`, and the `Releases map[string]*module.Instance` field — is deleted; it was unreachable dead code with no production caller.
**Migration**: None. External tools never consumed `pkg/bundle` in a supported flow. Bundle types are reintroduced under the Instance vocabulary only if bundle support is built.
