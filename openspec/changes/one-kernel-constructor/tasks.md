...
- [ ] 1.2 `task fmt`, `task lint`, `task test` green, then commit `refactor(config): add NewKernel as the single kernel construction site`
...
- [ ] 2.3 `task fmt`, `task lint`, `task test` green, then commit `refactor(config): construct every kernel through config.NewKernel`
