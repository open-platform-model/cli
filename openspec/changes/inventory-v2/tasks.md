...
## 2. Inventory persistence redesign

- [ ] 2.1 Redesign inventory persistence to store the ownership-only shape directly instead of the old history-bearing format.
- [ ] 2.2 Keep Secret naming/labels aligned with the new ownership-only inventory model only where still needed by the CLI.
- [ ] 2.3 Add tests covering ownership-only inventory persistence round trips.
...
- [ ] 3.2 Update delete/status/list flows to rely on ownership inventory for resource enumeration, while removing dependencies on inventory change-history metadata.
...
- [ ] 4.2 Add focused workflow coverage for apply/delete/status/list behavior using ownership-only inventory.
