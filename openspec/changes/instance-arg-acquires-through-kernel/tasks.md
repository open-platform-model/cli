...
- [ ] 1.2 `task fmt`, `task lint`, `task test` green, then commit `refactor(cmdutil): add InstanceDir and use it in the render workflow`
...
- [ ] 2.3 `task fmt`, `task lint`, `task test` green and the e2e instance tests that delete by identifier (`tests/e2e/instance_operator_owned_test.go`) green, then commit `fix(instance): resolve the instance argument through the kernel`
...
- [ ] 3.2 `task fmt`, `task lint`, `task test` green, then commit `refactor(loader): delete the instance file loader copy`
