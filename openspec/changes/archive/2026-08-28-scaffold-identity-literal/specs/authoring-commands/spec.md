...
The writer SHALL tolerate every authored form (plain literal, `#VersionType & "x.y.z"` assertion chain, defaulted disjunction) and preserve the form it finds. Every identity file the CLI itself emits (scaffold, repair, official templates) SHALL use the plain-literal form; no CLI code path SHALL introduce a default arm into an identity `Version`.
...
#### Scenario: Plain literal rewritten in place

- **WHEN** the field is declared `"1.2.0"` and `version set 1.3.0` runs
- **THEN** the result is `"1.3.0"` with every other byte of the file unchanged
