...
- [ ] 1.3 Add unit tests for `pathRewrittenError`: verify `Path()` returns rewritten path, verify other methods delegate correctly, verify `formatCUEDetails` renders rewritten paths.
...
- [ ] 2.2 Add unit tests for `findSourcePosition`: single-source value returns `Pos()` directly, unified value returns a valid position from one of the parts, value with no position returns `token.NoPos`.
...
- [ ] 4.1 Update `TestValidateValuesAgainstConfig` — existing test cases should pass with updated error paths (`values.X` instead of `#config.X`). Update assertions accordingly.
- [ ] 4.2 Add test: top-level disallowed field emits error with `values.` path and source position.
- [ ] 4.3 Add test: nested disallowed field inside pattern-constrained struct emits error with full path (e.g., `values.media.tvshows.badField`) and source position.
- [ ] 4.4 Add test: pattern constraint fields are accepted (no false "field not allowed" for `media.tvshows` when `[Name=string]` pattern exists).
- [ ] 4.5 Add test: type mismatch at nested level (string where struct expected) emits error with correct path and does not recurse.
- [ ] 4.6 Add test: optional fields in schema are accepted without error.
- [ ] 4.7 Add test: deeply nested struct (3+ levels) validates correctly with full path in errors.
- [ ] 4.8 Add test: multi-source unified value attributes error position to the correct source file via `Expr()` decomposition.
...
- [ ] 5.3 Verify the jellyfin example module: run `opm mod vet examples/jellyfin/ -f examples/jellyfin/values.cue -f examples/jellyfin/val.cue` and confirm errors show `values.` paths with file:line:col positions.

## 6. Validation gates

- [ ] 6.1 Run `task fmt` — all Go files formatted.
- [ ] 6.2 Run `task test` — all tests pass (including updated error assertions).
- [ ] 6.3 Run `task check` — fmt + vet + test all pass.
