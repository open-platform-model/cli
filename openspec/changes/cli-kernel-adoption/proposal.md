...
- `status.lastAppliedRenderDigest` adopts the operator's exact digest algorithm/serialization; `lastAppliedSourceDigest` stays the reference-identity digest — implementation investigation showed the operator's CUE-native path uses the same `ModuleSourceDigest(path@version)`, so C1's computation was already operator-parity.
