...
<!-- X3 (enhancement 0002 D6): the `release`/`rel` command group is renamed to `instance`/`inst`, the package moves `internal/cmd/release/` → `internal/cmd/instance/`, the import alias `cmdrelease` → `cmdinstance`, and the constructor `NewReleaseCmd` → `NewInstanceCmd`. Only the command *verb* and its wiring change here. The `--release-name` flag names, the `#ModuleRelease`/`#ModuleInstance` core-type prose, and the `module-synthetic-release` capability cross-ref are intentionally left to their owning slices (X4 / C1-X1) to avoid double-editing. -->
...
- **AND** the `build` subcommand SHALL synthesize a `#ModuleRelease` from a module-package directory and render it through the shared instance-render pipeline
...
The `opm instance build` subcommand (when used with a directory argument), the `opm module build` subcommand, and the `opm module apply` subcommand SHALL accept a `--name <string>` flag that overrides the synthetic `metadata.name`. Defaults are described in the `module-synthetic-release` capability spec.
...
The subcommand SHALL synthesize a `#ModuleRelease` from the directory (reusing the `module-synthetic-release` capability), render the result through the same pipeline as `opm instance apply`, and apply the produced resources to a Kubernetes cluster with full inventory, prune, dry-run, and ownership semantics.
